...
namespace WinAppDriverUIRecorder
{
    ...
    internal class MouseKeyboardEventHandler
    {
        // keyboard state
        private static List<byte> s_listRecordedKeycode = new List<byte>();

        private static int s_keyboardInputTick;
        private static VirtualKeys s_lastKeyCode = VirtualKeys.VK_RETURN;
        private static bool s_lastKeyDown = false;
        private static bool s_bCapsLock = false;
        private static bool s_bNumLock = false;
        private static bool s_bScrollLock = false;

        // mosue events
        private static int tickLeftUp = 0;

        private static MouseState mouseState = MouseState.LeftMouseUp;
        private static System.Drawing.Point ptCursorDown = new System.Drawing.Point(0, 0);
        private static System.Drawing.Point ptCursorUp = new System.Drawing.Point(0, 0);

        // recording thread
        public static Thread s_threadRecorder = null;

        private static ManualResetEvent s_eventQuitRecording = new ManualResetEvent(false);
        private static ManualResetEvent s_eventRecordNow = new ManualResetEvent(false);
        private static System.Timers.Timer s_timerFromPoint = null;

        private static System.Drawing.Point ptUiWalking = new System.Drawing.Point(0, 0);
        private const int nDoubleClickDelta = 750;
        private const int nDelayRecord = 125;
        private const int nMinDist = 15;
        private static string s_strXmlNodes = null;
        private static object s_lockUiPath = new object();
        private static int s_nPathHash = 0;
            ...
            while (true)
            {
                ...
                if (bStartWalk && MouseKeyboardHook.s_bPauseMouseKeyboard == false)
                {
                    ...
                    if (dist > nMinDist)
                    {
                        ...
                        Console.WriteLine($"GetUiXPath: {stopTime}");
        ...
        private static void PublishKeyboardInput()
        {
